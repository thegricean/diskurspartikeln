...
In this paper, we present a series of experiments suggesting that the epistemic necessity modal \emph{must} in utterances such as \emph{It must be raining} receives its weaker interpretation by non-truth-conditional means of pragmatic reasoning rather than by components of its lexical semantics. Specifically, we demonstrate to what extent the interpretation of epistemic \emph{must} differs from the interpretation of statements involving weaker modals or no modals at all. To substantiate our claim that the interpretation of epistemic \emph{must} is not part of the descriptive part of the utterance, we included German discourse particles in our experiments and show that epistemic \emph{must} in German (`muss’) patterns with this non-propositional means of expressing that the speaker commitment to the proposition is weakened.	
\end{abstract}
...
At issue is the failed inference from (\ref{must}) to (\ref{bare}) first observed by \cite{karttunen1972}: How could \emph{must p} not entail \emph{p}? If it is necessarily raining, then surely it is raining. It appears that talking about what is \emph{necessarily} the case commits speakers to less than does talking about what is \emph{actually} the case. \cite{karttunen1972} claimed that the inference from (\ref{must}) to (\ref{bare}) fails because \emph{must p} is a weaker statement than bare \emph{p}. In other words, \emph{must} is weak. 
...
What is interesting about 2d in our context is that discourse particles such as \emph{wohl} are not part of the propositional, truth-conditional content of an utterance, in contrast to modal adverbs such as \emph{vermutlich} (‘presumably’), cf. Gutzmann (2015: 215-268). Specifically, for \emph{wohl} Zimmermann (2004, 2008) has shown that this particular particle cannot affect the presuppositional domain of utterance interpretation, unlike modal adverbs such as \emph{vermutlich.}

In this paper, we provide evidence for the claim that epistemic \emph{must} should not be analyzed as contributing truth-conditional content (as, e.g., in the account in terms of quantification over possible worlds, cf. Fintel and Gillies, 2007). In order to substantiate our claim, we not only compare the interpretation of epistemic \emph{must} to statements involving weaker modals or no modals at all; rather, given our cross-linguistic perspective, we additionally hypothesize that in the domain of speaker commitment, epistemic \emph{must} patterns with non-truth-conditional means of expressing weakened commitment such as discourse particles in German.
